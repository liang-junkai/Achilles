...
#include <wolfssl/wolfcrypt/error-crypt.h>
    ...
    /* set NO_WRAPPERS before headers, use direct internal f()s not wrappers */WC_NO_RNG
...
#endif
void print_x2(byte *cipher,int len){
    for(int i=0;i<len;i++){
        printf("%02X",cipher[i]);
    }  
    printf("\n");
                    ...
                    byte **output, byte padValue)
{
    ...
    word16 i;
    printf("In RsaUnpad, pkcs blocklen: %d\n",pkcsBlockLen);
    if (output == NULL || pkcsBlockLen < 2 || pkcsBlockLen > 0xFFFF) {
        return BAD_FUNC_ARG;
    }

    if (padValue == RSA_BLOCK_TYPE_1) {
        WOLFSSL_MSG("RSA_BLOCK_TYPE_1");
...
static int RsaFunctionPrivate(mp_int* tmp, RsaKey* key, WC_RNG* rng)
{
    //printf("RSA private\n");
    byte output[256];
    mp_to_unsigned_bin(tmp,output);
    //printf("in rsaprivate\n");
    //print_x2(output,mp_bitsused(tmp)/8+1);
    int    ret = 0;
...
#error wolfSSL does not handle building bind support with async crypto
    ...
    case RSA_STATE_ENCRYPT_EXPTMOD:
        ...
        key->dataLen = outLen;
        //printf("in enc before func\n");
        //print_x2(out,sz);
        ret = wc_RsaFunction(out, (word32)sz, out, &key->dataLen, rsa_type, key,
                             rng);
        //printf("in enc after func\n");
        //print_x2(out,key->dataLen);
                                                     ...
                                                     RsaKey* key, WC_RNG* rng)
{
    ...
    ret = RsaPublicEncryptEx(in, inLen, out, outLen, key,
        RSA_PUBLIC_ENCRYPT, RSA_BLOCK_TYPE_2, WC_RSA_NO_PAD/*WC_RSA_PKCSV15_PAD*/,
...
int wc_RsaPrivateDecryptInline(byte* in, word32 inLen, byte** out, RsaKey* key)
{
    ...
    ret = RsaPrivateDecryptEx(in, inLen, in, inLen, out, key,
        RSA_PRIVATE_DECRYPT, RSA_BLOCK_TYPE_2, WC_RSA_NO_PAD/*WC_RSA_PKCSV15_PAD*/,
        WC_HASH_TYPE_NONE, WC_MGF1NONE, NULL, 0, 0, rng);
    RESTORE_VECTOR_REGISTERS();
    return ret;
}
