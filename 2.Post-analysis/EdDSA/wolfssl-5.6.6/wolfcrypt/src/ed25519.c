...
void set_bit(byte *target,int i){
    int pos=i/(sizeof(byte)*8);
    int num=i%(sizeof(byte)*8);
    byte tmp=1;
    for(int k=0;k<num;k++){
                            ...
                            const byte* context, byte contextLen)
{
    ...
    *outLen = ED25519_SIG_SIZE;
    ...
    ret = ed25519_hash(key, key->k, ED25519_KEY_SIZE, az);
    //printf("hash r: \n");
    //print_x(az,ED25519_PRV_KEY_SIZE);
    if (ret != 0)
        return ret;
...
int begin,end;
    ...
    //     print_x(hram,WC_SHA512_DIGEST_SIZE);
    // }
    ...
    static const byte ed25519_order2[32] = {
    ...
    // sc_reduce(d);
    // fprime_mul(d2,d,e,ed25519_order2);
    // fprime_sub(s1,d2,ed25519_order2);
    XMEMCPY(out + (ED25519_SIG_SIZE/2),s1,(ED25519_SIG_SIZE/2));
    // printf("find bit: %d\n",cnt);
    // cnt++;
#endif
#endif /* WOLFSSL_SE050 */
...
int wc_ed25519_sign_msg_ex2(const byte* in, word32 inLen, byte* out,
                            word32 *outLen, ed25519_key* key, byte type,
                            const byte* context, byte contextLen, byte *sec)
{
    int begin,end;
    //begin=rdtsc();
    //printf("in ed25519 2\n");
...
begin=rdtsc();
    ...
    byte s1[32],s2[32];
    printf("The Program cost %d CPU cycles\n", end-begin);
    sc_muladd(s1, hram, az, nonce);
    sc_muladd(s2, hram, az, nonce);
    static const byte ed25519_order2[32] = {
    0xed, 0xd3, 0xf5, 0x5c, 0x1a, 0x63, 0x12, 0x58,
    0xd6, 0x9c, 0xf7, 0xa2, 0xde, 0xf9, 0xde, 0x14,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    XMEMCPY(out + (ED25519_SIG_SIZE/2),s1,(ED25519_SIG_SIZE/2));
    //return ret;
    byte out2[ED25519_SIG_SIZE];
    XMEMCPY(out2,out,ED25519_SIG_SIZE);
    int flag=0;
    for(int cnt=0;cnt<252;cnt++){
        byte d[64],d2[32];
        XMEMSET(d, 0, sizeof(d));
        XMEMSET(d2, 0, sizeof(d2));
        byte e[64];
        ...
        fprime_mul(d2,d,e,ed25519_order2);
        XMEMCPY(s2,s1,32);
        fprime_sub(s2,d2,ed25519_order2);
        int verified=0;
        byte message[] = {"hello world"};
        XMEMCPY(out2 + (ED25519_SIG_SIZE/2),s2,(ED25519_SIG_SIZE/2));
        //printf("out: \n");
        //print_x(out,*outLen);
        ret = wc_ed25519_verify_msg(out2, *outLen, message, sizeof(message), &verified,
                                    key);
        if (ret < 0||verified==0)
        {
            fprime_add(s2,d2,ed25519_order2);
            fprime_add(s2,d2,ed25519_order2);
            XMEMCPY(out2 + (ED25519_SIG_SIZE/2),s2,(ED25519_SIG_SIZE/2));
            int verified2=0;
            ret = wc_ed25519_verify_msg(out2, *outLen, message, sizeof(message), &verified2,
                                    key);
            if(ret >=0&&verified2){
                flag=1;
                printf("find bit, pos :%d value: 1\n",cnt);
                break;
            }
        }
        else if (verified != 0)
        {
            printf("find bit, pos :%d value: 0\n",cnt);
            flag=1;
            break;
        }
    }
...
int wc_ed25519_sign_msg_ex3(const byte* in, word32 inLen, byte* out,
                            word32 *outLen, ed25519_key* key, byte type,
                            const byte* context, byte contextLen, byte *sig,byte *unique,byte* leakage)
{
    //printf("in ed25519 2\n");
    int    ret;
#ifdef WOLFSSL_SE050
    ...
    *outLen = ED25519_SIG_SIZE;
    ...
    byte s1[32],s2[32];
    XMEMCPY(s1,sig+(ED25519_SIG_SIZE/2),(ED25519_SIG_SIZE/2));
    XMEMCPY(s2,sig+(ED25519_SIG_SIZE/2),(ED25519_SIG_SIZE/2));
    // sc_muladd(s1, hram, az, nonce);
    // sc_muladd(s2, hram, az, nonce);
    static const byte ed25519_order2[32] = {
    0xed, 0xd3, 0xf5, 0x5c, 0x1a, 0x63, 0x12, 0x58,
...
int wc_ed25519_sign_msg2(const byte* in, word32 inLen, byte* out,
                        word32 *outLen, ed25519_key* key,byte* sec){
    return wc_ed25519_sign_msg_ex2(in, inLen, out, outLen, key, (byte)Ed25519,
                            NULL, 0,sec);
                        }
int wc_ed25519_find_bit(const byte* in, word32 inLen, byte* out,
                        word32 *outLen, ed25519_key* key,byte *sig,byte* unique,byte* leakage){
    return wc_ed25519_sign_msg_ex3(in, inLen, out, outLen, key, (byte)Ed25519,
                            NULL, 0,sig,unique,leakage);
                        }
/*
    in          contains the message to sign
    inLen       is the length of the message to sign
