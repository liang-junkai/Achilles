...
uint64_t number_of_seconds_to_hammer = 360000;
...
uint64_t GetGFN (uint8_t* virtual_address) {
  // Read the entry in the pagemap.
  uint64_t value;
  int got = pread(pagemap, &value, 8,
                  (reinterpret_cast<uintptr_t>(virtual_address) / 0x1000) * 8);
...
size_t get_dram_mapping(void* phys_addr_p) {
  ...
  static const size_t h0[] = { 16, 20 };
  static const size_t h1[] = { 15, 19 };
  static const size_t h2[] = { 14, 18 };
  static const size_t h3[] = { 13, 17 };

  size_t hash = 0;
  size_t count = sizeof(h0) / sizeof(h0[0]);
  for (size_t i = 0; i < count; i++) {
    hash ^= (phys_addr >> (h0[i]-1)) & 1;
  }
  size_t hash1 = 0;
  count = sizeof(h1) / sizeof(h1[0]);
  for (size_t i = 0; i < count; i++) {
    hash1 ^= (phys_addr >> (h1[i]-1)) & 1;
  }
  size_t hash2 = 0;
  count = sizeof(h2) / sizeof(h2[0]);
  for (size_t i = 0; i < count; i++) {
    hash2 ^= (phys_addr >> (h2[i]-1)) & 1;
  }
  
  size_t hash3 = 0;
  count = sizeof(h3) / sizeof(h3[0]);
  for (size_t i = 0; i < count; i++) {
    hash3 ^= (phys_addr >> (h3[i]-1)) & 1;
  }
...
uint64_t HammerAddressesStandard(
  ...
  //printf("va1: %lx, va2: %lx.\t", (uint64_t) f, (uint64_t) s);
...
uint64_t HammerAllReachablePages(void* memory_mapping, uint64_t memory_mapping_size, HammerFunction* hammer,
  ...
  FILE *fp=fopen("bit_flip_addr.txt", "w");
...
#define VAL (0)
#define VAL1 (-1ULL)
	for(int bit_flag=0;bit_flag<2;bit_flag++){
        ...
        for (uint8_t* target_page8 : pages_per_row[row_index+offset]) {
          ...
          for (uint32_t index = 0; index < 512; ++index)
            target_page[index] = (bit_flag==0? VAL: VAL1);
        }
	...
	std::pair<uint64_t, uint64_t> first_page_range(
        ...
        for (const uint8_t* target_page8 : pages_per_row[row_index+offset]) {
          ...
          for (uint32_t index = 0; index < 512; ++index) {
          uint64_t value= (bit_flag==0? VAL: VAL1);
            if (target_page[index] != value) {
              ++number_of_bitflips_in_target;
              fprintf(stdout,"[!] Found %zu. flip (0x%016lx != 0x%016lx) in row %ld (%lx) (when hammering "
                  "rows %zu and %zu at first offset %zu and second offset %zu\n", number_of_bitflips_in_target, target_page[index], value, row_index+offset,
