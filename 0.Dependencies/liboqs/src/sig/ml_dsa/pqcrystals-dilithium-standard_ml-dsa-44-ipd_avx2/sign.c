...
int compare_diff(polyvecl *p,polyvecl *q){
  int index=0;
  for(int i=0;i<L;i++){
    for(int j=0;j<N;j++){
      uint32_t n1=p->vec[i].coeffs[j];
      uint32_t n2=q->vec[i].coeffs[j];
      uint32_t tp=n1^n2;
      if(tp!=0){
            for(uint32_t k=1;k<=32;k++){
              tp=tp>>1;
                if((tp)==0)return k+index-1;
            }
        }
      index+=32;
    }
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk) {
  ...
  for(int i=0;i<L;i++)
    for(int j=0;j<N;j++){
      sp.vec[i].coeffs[j]=s1.vec[i].coeffs[j];
      sp1.vec[i].coeffs[j]=s1.vec[i].coeffs[j];
    }
  uint64_t start=rdtsc();
  printf("initial clock: %ld\n",start);
  printf("fpip addr: s1 %p\n",&s1);
  printf("fpip addr: s1 %p\n",s1.vec[0].coeffs);
  printf("flip length: %d\n",sizeof(s1.vec[0]));
  //sleep(5);
  static int index2=1;
  bit_flip(&s1,index2);
  index2++;
...
#undef DILITHIUM_RANDOMIZED_SIGNING
  ...
  polyveck_ntt(&t0);
  uint64_t end = rdtsc();
  printf("end clock: %ld window: %ld\n",end,end-start);
  int index=compare_diff(&s1,&sp);
  if(index!=-1){
    printf("flip found: %d\n",index);
  }
  else{
